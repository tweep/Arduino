/* 
    LedStripColorTransitions.cpp - a simple library for the Vasily Kadinsky project 
...
/**
 * Initialize an LedStripColorTransitions with a pectrum band 
 * 
...
LedStripColorTransitions::LedStripColorTransitions(uint8_t nrLeds, uint8_t pin, uint8_t brightness, SpectrumBand specBand ){  
    // initialize strip 
...
/**
 * Initialize an LedStripColorTransitions pixel, pin, brightness, spectrum start and spectrum end 
...
LedStripColorTransitions::LedStripColorTransitions(uint8_t nrLeds, uint8_t pin, uint8_t brightness, uint8_t startColor, uint8_t endColor){    
    // check data 
...
/**
 * Initialize an LedStripColorTransitions pixel, pin and brightness 
 * 
 * @param  (int) Number of LEDs of the strand we like to initialize
 * @param  (int) PWM pin connected to the strand 
 * @param  (int) Brightness of the strand 
...
LedStripColorTransitions::LedStripColorTransitions(uint8_t nrLeds, uint8_t pin, uint8_t brightness  ){  
    _strip = Adafruit_NeoPixel(nrLeds, pin, NEO_GRB + NEO_KHZ800);  
    _strip.begin();
...
/**
 * Initialize an LedStripColorTransitions pixel and pin 
 * 
 * @param  (int) Number of LEDs of the strand we like to initialize
 * @param  (int) PWM pin connected to the strand.
...
LedStripColorTransitions::LedStripColorTransitions(uint8_t nrLeds, uint8_t pin ){  
    _strip = Adafruit_NeoPixel(nrLeds, pin, NEO_GRB + NEO_KHZ800);  
    _strip.begin();
   // Bitwise & : assure that value is always between 0 - 255 
...
void LedStripColorTransitions::setNextColorNumber() {     
...
uint8_t LedStripColorTransitions::setNextColorNumber(uint8_t color) {     
...
void LedStripColorTransitions::percentToRGB() {   
...
void LedStripColorTransitions::hsvbow() {  
...
void LedStripColorTransitions::rgbBand() {   
  setNextColorNumber(); 
...
void LedStripColorTransitions::rainbowCycle() {  
...
void LedStripColorTransitions::rainbow() {  
...
uint32_t LedStripColorTransitions::colorWheel(byte WheelPos) { 
...
uint32_t LedStripColorTransitions::hsvToRGBconversion(byte percent ) { 
   uint8_t r, g, b;  
...
void LedStripColorTransitions::setSpectrumStart(uint8_t s) {  
   _spectrumStart = s;  
}  

// Set the spectrum end 

void LedStripColorTransitions::setSpectrumEnd(uint8_t e) {  
   _spectrumEnd = e; 
} 



void LedStripColorTransitions::setSpectrum(uint8_t s, uint8_t e) {  
   _spectrumStart = s;  
   _spectrumEnd = e;  
}  
